      ...
      <ul className={`my-10 grid max-md:gap-x-5 md:gap-y-5 md:grid-rows-timeline max-md:grid-cols-timeline ${cols[experience.length]}`}>
        {experience.map((item, index) => (
          <li key={item.name} className="grid max-md:col-span-3 md:row-span-3 max-md:grid-cols-subgrid md:grid-rows-subgrid experience-row">
            <div className={`max-md:justify-self-end self-center experience-year ${bgAfter[index % 6]} ${index % 2 ? "md:row-start-3 md:self-start even" : "md:self-end"}`}>{item.dates ? <h4 className={`${borders[index % 6]} w-84 md:h-42 text-xxs text-center`}>{formatDates(item.dates)}</h4> : <h4 className={`${borders[index % 6]} w-42 h-42 text-reg`}>{item.start}</h4>}</div>
